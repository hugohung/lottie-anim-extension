...
def _calc_stagger_ref(i, H, pos_y):
    """参考动效错峰：每元素间隔 1-3f，高处元素先入场。
    返回错峰帧数（不含索引基础值，由调用方叠加）"""
    bonus = max(0, int((H - pos_y) / 180))
    return i * 2 + bonus  # i*2 约 1-2f 间隔
...
def stage_timeline(classify_out, params=None):
    """Stage 2: 参考动效风格时间轴
    结构：两段式交叉切换 + 首尾空帧循环
    - A组: t=0 从屏幕外入场 → 展示 ~60f → 退场到屏幕外
    - 交叉窗口: A退场 + B入场 同步
    - B组: t≈T/2 从屏幕外入场 → 展示 ~60f → 退场到屏幕外
...
def _build_pos_kfs_style(l, in_start, in_end, out_start, out_end, W, H, fps):
    ...
    # 退场蓄力时长（秒→帧），确保 out_windup_t < out_end
    out_windup_dur_f = min(windup_dur_f, max(2, (out_end - out_start) // 3))
...
def _build_opa_kfs_style(l, in_start, in_end, out_start, out_end, is_a_group, fps):
    ...
    # B组：快速淡入 → 展示 → 快速淡出
    is_deco = _is_decoration(l.get('nm', ''))
    # 淡入时长（秒→帧）：装饰类 0.07s 闪现，其他 0.10s
    fade_f = _s2f(0.07 if is_deco else 0.10, fps)
    ei = EASE_BOUNCE_I if is_deco else EASE_STD_I
    eo = EASE_BOUNCE_O if is_deco else EASE_STD_O
